...
# TODO: Benchmark config parameter are hardcoded here
# Add string "jumboframes" to the config to test with jumboframes
BENCHMARK_CONFIGS = [
    {"test_name": "test name", 
     "amount_threads": 2,
     "jumboframes": False,
...
DEFAULT_PARAMETER = f" -i0 -u --enhanced --reportstyle=C --sum-only"
MTU_MAX = 9000
MTU_DEFAULT = 1500
...
def run_test_server(config: dict, test_name: str, file_name: str, ssh_server: str, results_folder: str, env_vars: dict) -> bool:
    ...
    command_str = f"{PATH_TO_BINARY} -s {DEFAULT_PARAMETER} -w {config['parameter']['-w']} -t {int(config['parameter']['-t']) + 4}"
    ...
    if server_output:
        ...
        # Additionally add field: amount_threads, mode, ip, test_name, mss, recv_buffer_size, send_buffer_size, test_runtime_length
        # Output file: iperf2-server-{csv_file_name}
    ...
    if server_error:
        ...
        if ssh_server is None:
            ...
            with open(log_file_path, 'a') as log_file:
                log_file.write("Test: " + test_name + " Run: " + config["test_name"] + '\n')
                log_file.write("Config: " + str(config) + '\n')
...
def run_test_client(config: dict, test_name: str, file_name: str, ssh_client: str, results_folder: str, env_vars: dict) -> bool:
    # Output file: iperf2-client-{csv_file_name}
    logging.info(f"{test_name}: Running iperf2 client on {ssh_client}")
    # Add parameter: --bandwidth=100G
    return True
...
def setup_remote_repo_and_compile(ssh_target, path_to_repo):
    ...
    execute_command_on_host(ssh_target, f'cd {path_to_repo} && git pull && git checkout {IPER2_VERSION}')
    # Compile the binary
